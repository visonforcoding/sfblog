/*
 * MoonCake v1.1 - UI Components Demo JS
 *
...
;(function( $, window, document, undefined ) {
	...
	$(document).ready(function() {
		...
		if( $.fn.progressbar ) {
			...
			demos.stripedPb( $('#demo-pb-02') );
			demos.activePb( $('#demo-pb-03') );
			
			demos.pbTypes( );

			$( '#jui-pb-rand' ).on( 'click', function( e ) {
				for( var i = 1; i <= 7; i++ ) {
					$( '#demo-pb-0' + i ).progressbar( 'value', Math.floor(Math.random() * 80) + 20 );
				}
			});
		}
		...
		if( $.fn.miniColors ) {
			...
			$( '.minicolors-component' ).miniColors({
				opacity: true, 
				wrapperClassName: 'add-on'
			});
